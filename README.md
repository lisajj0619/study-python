...
3. unpacking
    매개변수 *로 시작하면 kwargs 형식과 동일하게 받아야 하고,
...
# 8. 데코레이터(장식)
    ...
    주변 로직을 다른 함수에 분리시켜 놓은 뒤 메인 로직이 실행될 때 함께 실행되도록 할 수 있다.
...
### 클래스의 필드 사용
    1. 객체화(instance) - 추상적인 것을 구체화 시킨것
...
### 패키지
    ...
    하지만, 하위 버전(3.3 미만에서도 인식되기 위해서는 직접 생성해 놓는 것이 좋다.
